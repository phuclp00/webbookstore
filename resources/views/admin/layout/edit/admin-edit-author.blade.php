...
<div id="content-page" class="content-page">
   <div class="container-fluid">
      <div class="row">
         <div class="col-sm-12">
            <div class="iq-card">
               <a href="{{route('admin.author')}}"> <button type="button" class="btn btn-info btn-fixed"> Return
                     back</button></a>
               <div>
                  <img class="img-thumbnail rounded"
                     src="{{asset($data->image==null?"/images/books/default.jpg":$data->image)}}" alt="Author Image"
                     style="margin-top: 20px;padding: 20px">
               </div>
               <div class="iq-card-header d-flex justify-content-between">
                  <div class="iq-header-title">
                     <h4 class="card-title">Edit Author: {{$data->name}}</h4>
                  </div>
               </div>
               @include('post.create')
               <div class="iq-card-body">
                  <form id="edit_form" action="{{route('admin.author.edit',[$data->id])}}" method="post"
